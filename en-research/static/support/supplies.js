var categories1 = {
  "Medical/Nursing Supplies": ['Crutch', '車椅子', '歩行器', '大人用おむつ'],
  "食料品": ['パン', 'カップめん', '水', 'ベビーフード'],
  "幼児用品": [ '紙おむつ', 'ミルク', 'ベビーベッド'],
  "その他": ['衣服品', '靴', 'トイレットペーパー', '毛布', '携帯トイレ','テキスト入力'],
};
var categories2 = {
    "Crutch": ['SS(121~137cm)', 'S(137~157cm)','M (157-178cm)','L(178~198cm)'],
    "車椅子": ['大型', '中型', '小型'],
    "歩行器": ['サークル型', '四輪歩行車'],
    "大人用おむつ": ['S', 'M','L'],
    "衣服品": ['上着', 'ズボン'],
...
$(document).ready(function(){
  $("#category").change(function(){
    ...
    selectElement += '<option hidden>Please Select Item &#9660</option>';
      $("#label-1").html('<label for="category">Item2:</label>');
    ...
    $("#category1 select").change(function(){
      ...
      if (Category2 === "テキスト入力") {
        ...
        $("#label-3").html('<label for="category">Quantity:</label>').show();
        ...
        $("#label-2").html('<label for="category">Remarks:</label>').show();
        $("#category2").html('<input type="text" class="form-control" name="category2">');
        $("#label-3").html('<label for="category">Quantity:</label>').show();
        $("#category3").html('<input type="number" id="quantity" name="quantity" min="1"><br><br><button class="btn btn-primary" type="submit" onclick="return validate()">Send</button>');
      } else {
        selectElement2 += '<option hidden>Please Select Item &#9660</option>';
        $("#label-2").html('<label for="category">Item 3:</label>').show();
      ...
      $("#category2 select").change(function(){
        ...
        if (options3 === undefined || options3.length === 0) {
          $("#label-3").html('<label for="category">Remarks:</label>').show();
          $("#category3").html('<input type="text" class="form-control" name="category3">');
          $("#label-4").html('<label for="category">Quantity:</label>').show();
          $("#category4").html('<input type="number" id="quantity" name="quantity" min="1"><br><br><button class="btn btn-primary" type="submit" onclick="return validate()">Send</button>');
        } else {
          selectElement3 += '<option hidden>Please Select Item &#9660</option>';
