$(document).ready(function () {
    ...
    switch (selectedSubCategory1) {
      ...
      case 'アレルギー食品':
        $('#itemDetails').html('<input type="checkbox" name="allergen" value="えび">えび' +
          ...
          'その他：<input type="text" name="others" id="itemDetailsText">');
        break;
      case '衣服品':
      ...
      case '靴':
        ...
        break;
      case 'テキスト入力':
        $('#itemDetails').html('テキスト入力：<input type="text" name="others" id="itemDetailsText">');
        break;
      default:
        $('#itemDetails').html('');
        $('#subSubCategory').html('');
  ...
  function updateItemName() {
    var itemName = selectedCategory;
    if (selectedSubCategory1) {
        itemName += ' - ' + selectedSubCategory1;
    }
    if (selectedSubCategory2) {
        itemName += ' - ' + selectedSubCategory2;
    }

    // item_name フィールドの値を更新
    $('#id_item_name').val(itemName);  // id_item_name はフィールドの実際のHTML要素ID
}

  // 各選択肢が変更されるたびに更新
  $('input[type=radio][name=category], input[type=radio][name=subCategory], input[type=radio][name=subSubCategory], input[type=radio][name=sub3Category]').change(function () {
    updateItemName();
  });
});
